...
def find_best():
    best = 0
    def search(opened, released, current_valve, time_left):
        nonlocal best
        if released > best:
            best = released
        if time_left <= 0:
        ...
        if current_valve not in opened:
            search(opened.union([current_valve]), released + valves[current_valve]['flow_rate'] * time_left, current_valve, time_left-1)
        else:
            for k in [x for x in bfs_valves[current_valve].keys() if x not in opened and x in useful_valves]:
                search(opened, released, k, time_left-bfs_valves[current_valve][k])
